...
document.addEventListener("DOMContentLoaded", function() {
    checkEmptyList('.recipes-container');
    checkEmptyList('.shared-container');
    checkEmptyList('.favorite-container');
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response && response.recipeDetails && response.recipeDetails.length > 0) {
                ...
                // Função para adicionar o card à lista após o carregamento de todas as imagens
                function addCardToDOM(cardHtml) {
                    $("#FavoriteList").append(cardHtml);
                    console.log(cardHtml)
                    imagesLoaded++;

                    // Se todas as imagens foram carregadas, ajuste a altura conforme necessário
                    if (imagesLoaded === response.recipeDetails.length) {
                        adjustCardHeight();
                    }
                }
    ...
    // Verificar se há fotos na receita
                    ...
                    // Criar contêiner para as tags de categoria com rolagem horizontal
                    var categoryContainer = $('<div>').addClass('category-container d-flex flex-nowrap overflow-auto mb-1').css({'width': '100%', 'height' : '5%'});
                    categoryContainer.append(categoryTags);
                ...
                // Caso não haja receitas na resposta
                var noRecipesMessage = '<p class="text-muted">Nenhuma receita encontrada.</p>';
                $("#FavoriteList").append(noRecipesMessage);
            }
        },
        error: function(xhr, status, error) {
            console.error('Erro na solicitação de receitas favoritas:', error);
    ...
    // Verificar se há fotos na receita
                ...
                // Caso não haja receitas na resposta
                var noRecipesMessage = '<p class="text-muted">Nenhuma receita encontrada.</p>';
                $("#SharedList").append(noRecipesMessage);
            }
        },
        error: function(xhr, status, error) {
            console.error('Erro na solicitação de receitas favoritas:', error);
...
function checkEmptyList(containerSelector) {
    var container = $(containerSelector);
    var cards = container.find('.card');

    if (cards.length === 0) {
        container.css('height', '40vh'); // Adicione esta linha para definir a altura para 40vh
